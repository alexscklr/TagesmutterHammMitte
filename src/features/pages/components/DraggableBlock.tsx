...
export const DraggableBlock: React.FC<{ block: PageBlock }> = ({ block }) => {
    ...
    const handleProps = drag ? drag.getHandleProps(block.id) : { draggable: false, onDragStart: () => undefined, onDragEnd: () => undefined };
    const dropProps = drag ? drag.getDropProps(block.id) : { onDragOver: () => undefined, onDrop: () => undefined };
